...
getTxOut(hs_filtered_TxDb, hs_case_tx, "CDS", "testdata/hs_CDS_test", "both")
getTxOut(hs_filtered_TxDb, hs_case_tx, "whole", "testdata/hs_whole_test", "gff3")
...
getTxOut(mm_filtered_TxDb, mm_case_tx, "UTR5", "testdata/mm_UTR5_test", "both")
getTxOut(mm_filtered_TxDb, mm_case_tx, "UTR3", "testdata/mm_UTR3_test", "fa")
...
test_that("getTxout detects incorrect inputs", {
  expect_error(getTxOut(mm_filtered_TxDb, mm_case_genes, "CDS", "mydata/mm_test", "both"), "Please ensure the transcript list contains transcript IDs and not gene IDs.")
  expect_error(getTxOut(mm_filtered_TxDb, hs_case_tx, "CDS", "mydata/mm_test", "both"), "Please ensure the transcript list and gff are of the same species.")
  expect_error(getTxOut(mm_filtered_TxDb, mm_noCDS_tx, "CDS", "mydata/mm_test", "both"), "no sequences of type \"CDS\" found in transcript list")
  expect_error(getTxOut(mm_filtered_TxDb, hs_case_tx, "CDS", "mydata/mm_test", "both"), "Please ensure the transcript list and gff are of the same species.")
  expect_error(getTxOut(mm_filtered_TxDb, mm_case_tx, "CDS", "mydata/mm_test", "Both"), "not appropriate format, please use \"fa\", \"gff3\" or \"both\"")
  expect_error(getTxOut(unknown_TxDb, unknown_tx, "CDS", "mydata/mm_test", "both"), "Species type is unknown. Please ensure gff and transcript list are either mouse or human")
  expect_error(getTxOut(mm_filtered_TxDb, mm_case_tx, "CSD", "mydata/mm_test", "both"), "not appropriate seq type. please use one of \"whole\", \"CDS\", \"UTR5\" or \"UTR3\"")
