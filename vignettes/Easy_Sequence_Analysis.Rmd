...
We see that the case sequences have a significantly higher GC content than the case sequences.
...
Biostrings::consensusMatrix(Biostrings::DNAStringSet(enriched_kmers), as.prob = TRUE) %>% .[1:4,] %>% seqLogo::seqLogo()
...
#consensusMatrix(DNAStringSet(pull(filter(df, df$group == 1), kmer)), as.prob = TRUE) %>% .[1:4,] %>% seqLogo::seqLogo()
#consensusMatrix(DNAStringSet(pull(filter(df, df$group == 2), kmer)), as.prob = TRUE) %>% .[1:4,] %>% seqLogo::seqLogo()
