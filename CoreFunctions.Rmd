...
```{r, }
#hs_gff <- GenomicFeatures::makeTxDbFromGFF(file = "data/Gencodedat/gencode.v33.annotation.gff3.gz", format = "gff3", organism = "Homo sapiens")
#seqlevelsStyle(hs_gff) <- "NCBI"
#mm_gff <- GenomicFeatures::makeTxDbFromGFF(file = "data/Gencodedat/gencode.vM20.annotation.gff3.gz", format = "gff3", organism = "Mus musculus")
...
mm_f_gff <- filter_Tx("data/Gencodedat/gencode.vM20.annotation.gff3.gz")
hs_f_gff <- filter_Tx("data/Gencodedat/gencode.v33.annotation.gff3.gz")
```
...
make_longest_df <- function(TxDb_gff){ 
  ...
  longest_tx <- longest_tx %>% 
    group_by(gene_id) %>% 
    summarize(whole = dplyr::first(whole), 
              CDS = dplyr::first(CDS), 
