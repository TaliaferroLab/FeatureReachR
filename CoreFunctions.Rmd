...
#hs_gff <- GenomicFeatures::makeTxDbFromGFF(file = "data/Gencodedat/gencode.v33.annotation.gff3.gz", format = "gff3", organism = "Homo sapiens") # 11min 
...
mm_f_gff <- filter_Tx("data/Gencodedat/gencode.vM20.annotation.gff3.gz") #3.5 min
hs_f_gff <- filter_Tx("data/Gencodedat/gencode.v33.annotation.gff3.gz") #1.5 min
...
RNA_breakdown <- function(gff, tx_list){
  ...
  RNA_breakdown <- gff@elementMetadata %>% 
    ...
    dplyr::summarize(percent = n() / nrow(.) * 100)
  
  return(RNA_breakdown)
}

RNA_breakdown("data/Gencodedat/gencode.vM20.annotation.gff3.gz", mm_tx)
...
getTxOut <- function(TxDb_gff, tx_list, seq_type, file_name, output_type){
  ...
  if (output_type == "fa"){
    ...
    rtracklayer::export(tx_gff, paste(file_name, ".gff3", sep = ""), format = "GFF")
